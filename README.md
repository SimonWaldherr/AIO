...
- Support multiple AI providers (OpenAI, Anthropic, Google Gemini, Ollama, custom endpoints)
- Configure a **Primary and Secondary LLM** with per-task-type routing
- Perform **live web search** (Wikipedia or Brave Search) inside tasks via a tool-loop
- Attach **data files** (text, CSV, JSON, Markdown…) as context for every task
- Attach **images** for multimodal (vision) analysis with GPT-4o, Claude 3+, or Gemini 1.5+
...
- **Google (Gemini)** — Gemini 1.5 Pro / Flash and other Gemini models via the OpenAI-compatible endpoint
...
### 🔀 Dual-LLM Profiles & Task Routing
Configure a **Primary** and **Secondary** LLM in Settings. Use the **Task Routing** tab to assign specific task types (research, analysis, creation, execution, review) to either profile. For example, send complex analysis to GPT-4o and fast research tasks to a cheaper model—automatically.

Each task item and result card shows a badge indicating which LLM handled it.

### 🌐 Web Search (Tool-Loop)
Enable **Web Search** in the task input panel. The AI can issue `[SEARCH: query]` markers; the orchestrator intercepts them, performs a real search (Wikipedia or Brave Search), feeds the results back, and lets the AI continue. Up to 3 search iterations per task.

- **Wikipedia** — free, no API key required
- **Brave Search** — high-quality results, API key required (configure in Settings → Tools & Search)

### 📎 Data File Attachments
Click **File** in the Attachments section to attach `.txt`, `.csv`, `.json`, `.md`, `.xml`, `.yaml`, or `.yml` files (up to 500 KB each). Their content is injected into the system context for every task in the run. Remove individual files with the ✕ button.

### 🖼️ Vision / Multimodal
Click **Image** in the Attachments section to attach any image file. The image is sent (as base64) with the first user message to a vision-capable model. Works out of the box with:
...
1. Select your **AI Provider** (OpenAI, Anthropic, Google, Ollama, or Custom)
2. Enter your **API Key** (not required for local Ollama)
3. Optionally set a custom **Base URL** (e.g., `http://localhost:11434` for Ollama)
4. Optionally specify a **Model** name (e.g., `gpt-4o`, `claude-3-5-sonnet-20241022`, `gemini-2.0-flash`)

Click **Settings** (⚙️) in the navbar to change these at any time.

### 3. (Optional) Configure Secondary LLM & Task Routing
Open **Settings → Secondary LLM** to add a second AI provider. Then open **Settings → Task Routing** to choose which task types (research, analysis, creation, execution, review) use which LLM.

### 4. (Optional) Enable Web Search
Open **Settings → Tools & Search**, choose **Wikipedia** (free) or **Brave Search** (API key required), then tick **Enable Web Search** in the task input panel before starting.

### 5. Start Orchestrating
1. Enter your main task in the "Task Input" field
2. Optionally attach data files or an image
3. Optionally provide a custom system prompt
4. Choose your execution mode (Serial or Parallel)
5. Enable additional features (code execution, canvas, web search)
6. Click **Start Orchestration** to begin
7. Use the **Stop** button to cancel at any time
8. Use the **Export** (↓) button to save results as Markdown
...
### Example 1: Research Project with Web Search
**Main Task**: "Research the latest trends in quantum computing and summarize key findings"
*Enable Web Search (Wikipedia)*

The orchestrator might break this down into:
1. Research current quantum computing hardware (searches Wikipedia)
2. Analyse recent algorithmic advances (searches Wikipedia)
3. Identify commercial players and investments
4. Compile a structured report

### Example 2: Vision-Based Analysis
**Main Task**: "Analyse this diagram and explain the architecture"
*Attach an image*

The orchestrator will include the image in every task's user message, allowing a vision model to describe and reason about it.

### Example 3: Data Analysis from File
**Main Task**: "Analyse the sales data and find the top 3 trends"
*Attach a CSV file*

The CSV content is automatically injected as context for every task.

### Example 4: Dual-LLM Workflow
**Settings**: Primary = GPT-4o, Secondary = Ollama (llama3.1)
**Routing**: research/execution → Secondary, analysis/creation/review → Primary

Fast tasks use the local model; complex tasks use the cloud model—automatically.

### Example 5: Data Analysis with Visualization
**Main Task**: "Analyse website traffic data and create visual charts"
...
Customise how the AI approaches your tasks by providing specific instructions:
...
- Canvas context for drawing and visualisation

### Web Search Tool Loop
When web search is enabled, the AI is instructed to include `[SEARCH: query]` markers. The orchestrator:
1. Detects the markers in the LLM response
2. Performs the search (Wikipedia / Brave)
3. Injects the results back into the conversation
4. Re-calls the LLM to continue — up to 3 times per task
...
- **Google Gemini**: Via Google's OpenAI-compatible API endpoint
...
- **Vision Support**: Base64-encoded image injection for multimodal models
- **Web Search Tool Loop**: Real-time information retrieval during task execution
...
- **File Reader API**: Client-side file reading for data and image attachments
...
6. **Web Search Not Working**: Check that the search provider is configured in Settings → Tools & Search
7. **Vision Not Working**: Ensure you are using a vision-capable model (gpt-4o, claude-3-*, gemini-1.5+)
...
5. Use a fast Secondary LLM (e.g., Ollama/Gemini Flash) for research/execution tasks and route complex tasks to the Primary LLM
...
- Google for Gemini AI models and their OpenAI-compatible API
